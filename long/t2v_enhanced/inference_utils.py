...
def fill_video_chunks(data, target_chunk):

    result = []
    last_valid_entry = None
    change_point = []
    for entry in data:
        if entry["video_chunk_index"]:
            change_point.append(entry["video_chunk_index"])
    
    for i in range(target_chunk + 1):
        for entry in data:
            if entry["video_chunk_index"] == i:
                last_valid_entry = entry
                break
        if last_valid_entry:
            filled_entry = copy.deepcopy(last_valid_entry)
            filled_entry["video_chunk_index"] = i
