...
class TextToVideoSDPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def vanilla_encode_prompts(self, prompts):
        '''
        text_encoder output the hidden states
        prompts are based on list
        '''
        with torch.no_grad():
            tokens = self.tokenizer(prompts, max_length=self.tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors='pt').input_ids.to(self.device)
    ...
    def _encode_prompt(
        ...
        img_cond_left: Optional[torch.FloatTensor] = None,
        img_cond_right: Optional[torch.FloatTensor] = None,
    ):
        r"""
        ...
        sub_objects = prompt["sub_objects"]
        layout_boxes = prompt["layout_boxes"]

        prompt = []
        layout_mask = []
        prompt.append(base_prompt)
        layout_mask.append([0, 0, 1, 1])
        for sub_object, layout_box in zip(sub_objects, layout_boxes):
            prompt.append(sub_object)
            layout_mask.append(layout_box)
        ...
        # prompt.insert(0, negative_prompt)
        # prompt_emb = self.vanilla_encode_prompts(prompt)
        # print(prompt_embeds.shape)
        if img_cond is not None:
            if img_cond.ndim == 2:
                img_cond = img_cond.unsqueeze(1)
            # print(prompt_embeds.shape, img_cond.shape)
            # img_cond = img_cond.repeat(1, 3, 1)
            # print(prompt_embeds.shape, img_cond.shape)
            if prompt_embeds.shape[0] != img_cond.shape[0]:
                img_cond = img_cond.repeat(prompt_embeds.shape[0] // img_cond.shape[0], 1, 1)
            # print(prompt_embeds.shape, img_cond.shape)
    ...
    def __call__(
        ...
        ** kwargs,
        ...
        # print(input_frames_conditioning.shape) 1,1,3,256,256
        ...
        if img_cond_resampler is not None and image is not None:
            ...
            width = image_for_conditioniong.shape[-2]
            # left = image_for_conditioniong[:,:,:width//2, :]
            # right = image_for_conditioniong[:,:,width//2:, :]
            image_enc = img_cond_encoder(image_for_conditioniong)
            # image_enc_left = img_cond_encoder(left)
            # image_enc_right = img_cond_encoder(right)
            img_cond = img_cond_resampler(image_enc, batch_size=bsz)
            # img_cond_left = img_cond_resampler(image_enc_left, batch_size=bsz)
            # img_cond_right = img_cond_resampler(image_enc_right, batch_size=bsz)
        ...
        prompt_embeds = self._encode_prompt(
            ...
            # img_cond_left=img_cond_left,
            # img_cond_right=img_cond_right,
        )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # down_block_res_samples *= controlnet_weight 
                down_block_res_samples = [controlnet_weight * sample for sample in down_block_res_samples]
                mid_block_res_sample *= controlnet_weight
