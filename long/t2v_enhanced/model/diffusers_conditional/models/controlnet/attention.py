...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        # 2.5 Cross Frame Attention
        if is_spatial_attention and use_reference_attention:
            self.norm2_5 = nn.LayerNorm(
                dim, elementwise_affine=norm_elementwise_affine)
            self.attn2_5 = Attention(
                query_dim=dim,
                heads=num_attention_heads,
                dim_head=attention_head_dim,
                dropout=dropout,
    ...
    def forward(
        ...
        # 2.5 Cross Frame Attention
        if self.attn2_5 is not None and reference_frames_features is not None:
            print(f"Using ref attention, K shape: {reference_frames_features.shape}")
            norm_hidden_states = self.norm2_5(hidden_states)
            attn_output = self.attn2_5(
                norm_hidden_states,
                encoder_hidden_states=reference_frames_features,
                attention_mask=encoder_attention_mask,
                **cross_attention_kwargs,
            )
            hidden_states = attn_output + hidden_states
