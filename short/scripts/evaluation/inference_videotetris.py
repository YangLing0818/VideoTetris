...
from funcs import load_model_checkpoint, load_prompts_from_json, load_image_batch, get_filelist, save_videos
...
def get_parser():
    ...
    ## videotetris
    parser.add_argument("--use_demo", type=bool, default=True, help="whether to use demo mode")
    return parser
...
def run_inference(args, gpu_num, gpu_no, **kwargs):
    ...
    prompt_list = load_prompts_from_json(args.prompt_file)
    ...
    for idx in range(0, n_rounds):
        ...
        for prompt in prompts:
            texts.append(prompt["basic_prompt"])
            layout_masks.append([0, 0, 1, 1])  

            for sub_object, layout_box in zip(prompt["sub_objects"], prompt["layout_boxes"]):
                texts.append(prompt["basic_prompt"]+" "+sub_object)
                layout_masks.append(layout_box)

        text_emb = model.get_learned_conditioning(texts)
        print(text_emb.shape)
        text_emb = list(zip(layout_masks, text_emb))
