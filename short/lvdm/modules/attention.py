...
class CrossAttention(nn.Module):
    ...
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0., 
        ...
        if self.relative_position:
            ...
            if XFORMERS_IS_AVAILBLE and temporal_length is None and not self.cross_attention_mode:
                self.forward = self.efficient_forward
            elif XFORMERS_IS_AVAILBLE and temporal_length is None and self.cross_attention_mode:
                self.forward = self.efficient_comp_forward
    ...
    def efficient_comp_forward(self, x, context=None, mask=None):
        ...
        # print(out.shape)  # 48*2560*320

        # Decompose
        if region_diffusion:
            H, W = round((out.shape[1]//1.6) ** 0.5) , round((out.shape[1]//1.6) ** 0.5 * 1.6)
            regions = torch.split(out, out.shape[0]//len(layout_masks), dim=0)
            comp_res = torch.zeros_like(regions[0])
            base_ratio = 0.3
            
            for index, (region, mask) in enumerate(zip(regions, layout_masks)):
